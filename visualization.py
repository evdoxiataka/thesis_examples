...
def homogenous_models_outputs(trace, posterior_predictive, slope, intercept, rt):
    ...
    ##
    az.plot_dist(trace['b'], color=c1, ax=ax[1], label="Probabilistic Model")
...
def hom_het_hi_reg_line(reactions, trace, trace_het, trace_hi, posterior_predictive, posterior_predictive_het, posterior_predictive_hi, slope, intercept):
    ...
    az.plot_dist(trace_het['a'][:,9], color=c3, ax=ax[0][3], label = "het prob")
    ...
    az.plot_dist(trace_hi['a'][:,9], color=c4, ax=ax[0][3], label = "hi prob")
    ...
    az.plot_dist(trace_het['b'][:,9], color=c3, ax=ax[1][3], label = "het prob")
    ...
    az.plot_dist(trace_hi['b'][:,9], color=c4, ax=ax[1][3], label = "hi prob")
    ...
    for name,group in groups:
        if str(name) in subs:
            ...
            ## homogenous prob
            hdi_hom = az.plot_hdi(reactions.Days.unique(), rt_hom_prob, color = c1, hdi_prob=0.97, ax = ax[2][idx], plot_kwargs={'label':'hom prob','alpha':0.5}, fill_kwargs={'alpha': 0.1})
            ## heterogenous prob
            ...
            hdi_het = az.plot_hdi(reactions.Days.unique(), rt_het_prob, color = c3, hdi_prob=0.97, ax = ax[2][idx], plot_kwargs={'label':'het prob','alpha':0.5}, fill_kwargs={'alpha': 0.1})
            ## hierarchical prob
            ...
            hdi_hi = az.plot_hdi(reactions.Days.unique(), rt_hi_prob, color = c4, hdi_prob=0.97, ax = ax[2][idx], plot_kwargs={'label':'hi prob','alpha':0.5}, fill_kwargs={'alpha': 0.1},backend_kwargs = {})
